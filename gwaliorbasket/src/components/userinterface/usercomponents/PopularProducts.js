...
export default function PoppularProducts(){
    ...
    function explorecategory(){
      return productdeals.map((item)=>{
        return(<div style={{width:'10%',background:" linear-gradient(180deg, rgba(98,76,17,1) 12%, rgba(185,145,49,1) 94%)",borderRadius:'10%',margin:3,display:'flex',alignItems:'center',flexDirection:'column',  padding: "10px 10px 0px 10px"}}>
            <div style={{fontFamily:'Poppins',  fontSize: xs ? 4 : sm ? 6 : md ? 8 : lg ? 14 : 14, fontWeight: "bolder",
              textAlign: "center",color:" rgba(155,116,32,1)",background:'#fff',borderRadius:'0.3rem',width:'60%',padding:3,marginBottom:"15%"}}>Trending</div>
            <div style={{textAlign:'center',fontSize:xs?4:sm?6:md?8:lg?16:16,color:'#fff',fontFamily:'Poppins',fontWeight:'bolder',marginBottom:'30%'}}>{item.productname.substring(0,12)+'...'}</div>
            <img src={`${ServerURL}/images/${item.image}`} style={{width:'60%'}}/></div>)
      })
    }
 return(<div>
  <div style={{fontFamily:'Poppins',fontSize:20,fontWeight:'bolder'}}>Most Popular Products</div>
